...
function jsonToMoonBit(json, typename, flatten = true, example = false, allOmitempty = false) {
  ...
  function parseStruct(depth, innerTabs, scope, omitempty, oldParents, slice) {
    ...
    if (flatten && depth >= 2) {
      ...
      fromJsonFn.push(parent || 'AutoGenerated', ' with from_json(json, path) {\n  match json {\n    {')
      ...
      for (let i in keys) {
        ...
        const matchType = mbtMatchType(scope, snakeCaseVarname, keyname)
  ...
  function mbtMatchType(scope, snakeCaseVarname, keyname) {
    ...
    switch (matchType) {
      case "struct":
        ...
        if (subMatchType === 'Bool' || subMatchType === 'Json' || subMatchType === 'struct') { return snakeCaseVarname }
