...
function jsonToMoonBit(json, typename, flatten = true, example = false, allOmitempty = false) {
  ...
  function parseStruct(depth, innerTabs, scope, omitempty, oldParents, slice) {
    ...
    const seenTypeNames = []
    const structNewConstructor = ['\n\npub fn ']
    const toJsonFn = ['\n\npub fn to_json(self : ']
    const toJsonFnOptionSection = []
    ...
    if (flatten && depth >= 2) {
      ...
      structNewConstructor.push(`${parentName}::new() -> ${parentName} {\n  {`)
      ...
      if (allJsonFieldNamesIdentical && !allOmitempty) {
        ...
        structNewConstructor.push('\n  }\n}')
        appender(structNewConstructor.join(''))
      } else {
        ...
        appender(toJsonFnOptionSection.join(''))
        appender('\n  json.to_json()\n}')
      }
      ...
      if (allJsonFieldNamesIdentical && !allOmitempty) {
        ...
        structNewConstructor.push('\n  }\n}')
        append(structNewConstructor.join(''))
      } else {
        ...
        append(toJsonFnOptionSection.join(''))
        append('\n  json.to_json()\n}')
      }
