...
function jsonToMoonBit(json, typename, flatten = true, example = false, allOmitempty = false) {
  ...
  function parseStruct(depth, innerTabs, scope, omitempty, oldParents, slice) {
    ...
    if (flatten && depth >= 2) {
      ...
      for (let i in keys) {
        ...
        const safeMoonBitVarname = makeSafeMoonBitVarname(typename, safeToUseKeyname ? keyname : '')
        if (!isToJsonFieldNameIdentical(safeMoonBitVarname, keyname)) { allJsonFieldNamesIdentical = false }
        appender(safeMoonBitVarname + ' : ')
        parent = typename
      ...
      } else if (allJsonFieldNamesIdentical && !allOmitempty) {
      ...
      for (let i in keys) {
        ...
        const safeMoonBitVarname = makeSafeMoonBitVarname(typename, keyname)
        if (!isToJsonFieldNameIdentical(safeMoonBitVarname, keyname)) { allJsonFieldNamesIdentical = false }
        append(safeMoonBitVarname + ' : ')
        parent = typename
        parseScope(scope[keys[i]], depth)
        const isOption = allOmitempty || (omitempty && omitempty[keys[i]] === true)
        if (isOption) {
  ...
  function updateAllBuilders(scope, safeMoonBitVarname, keyname, typename, parentName, allBuilders, isOption) {
    ...
    switch (matchType) {
      case 'struct':
        ...
        allBuilders.structNewConstructor.push(`\n    ${safeMoonBitVarname}: ${defaultValue()},`)
        if (isOption) {
          allBuilders.toJsonFn.push(`\n  match self.${safeMoonBitVarname} {\n    Some(${safeMoonBitVarname}) => json["${safeMoonBitVarname}"] = ${safeMoonBitVarname}.to_json()\n    _ => ()\n  }`)
        } else {
          allBuilders.toJsonFn.push(`\n  json["${keyname}"] = self.${safeMoonBitVarname}.to_json()`)
        }

        switch (subType) {
          case 'Bool':
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType}? = match json.get("${keyname}") {\n    Some(True) => Some(true)\n    Some(False) => Some(false)\n    Some(Null) | None => None\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Bool or Null"))\n  }`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType} = match json.get("${keyname}") {\n    Some(True) => true\n    Some(False) => false\n   _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Bool"))\n  }`)
            }
            break
          case 'Int64':
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType}? = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => Some(${safeMoonBitVarname}.to_int64())\n    Some(Null) | None => None\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Int64 or Null"))\n  }`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType} = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => ${safeMoonBitVarname}.to_int64()\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Int64"))\n  }`)
            }
            break
          case 'Int':
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType}? = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => Some(${safeMoonBitVarname}.to_int())\n    Some(Null) | None => None\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Int or Null"))\n  }`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType} = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => ${safeMoonBitVarname}.to_int()\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected Int"))\n  }`)
            }
            break
          case 'slice':
            // TODO: if (isOption) {...
            allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : Array[${typename}] = match json.get("${keyname}") {\n    Some(Array(${safeMoonBitVarname})) => ${safeMoonBitVarname}_array_from_json!(${safeMoonBitVarname})\n    e => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname}: expected Array: got \\{e}"))\n  }`)
            allBuilders.fromJsonMatchArrayHelpers.push(`\n\npub fn ${safeMoonBitVarname}_array_from_json(json : Array[Json]) -> Array[${typename}]!@json.JsonDecodeError {
  ...
  arr
          ...
          case 'struct':
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${typename}? = match json.get("${keyname}") {\n    Some(Object(${safeMoonBitVarname})) => Some(@json.from_json!(${safeMoonBitVarname}.to_json()))\n    Some(Null) | None => None\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected ${typename} or Null"))\n  }`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${typename} = match json.get("${keyname}") {\n    Some(Object(${safeMoonBitVarname})) => @json.from_json!(${safeMoonBitVarname}.to_json())\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected ${typename}"))\n  }`)
            }
            break
          case 'Json':
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType}? = json.get("${keyname}")`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType} = match json.get("${keyname}") {\n    Some(${safeMoonBitVarname}) => ${safeMoonBitVarname}\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected valid Json"))\n  }`)
            }
            break
          default:
            if (isOption) {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType}? = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => Some(${safeMoonBitVarname})\n    Some(Null) | None => None\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname} expected ${subType} or Null"))\n  }`)
            } else {
              allBuilders.fromJsonFn.push(`\n  let ${safeMoonBitVarname} : ${subType} = match json.get("${keyname}") {\n    Some(${subMatchType}(${safeMoonBitVarname})) => ${safeMoonBitVarname}\n    _ => raise @json.JsonDecodeError((path, "${parentName}::from_json:${safeMoonBitVarname}: expected ${subType}"))\n  }`)
            }
        }
        return
      default:
  ...
  // makeSafeMoonBitVarname converts a StructName to a valid MoonBit variable name.
  function makeSafeMoonBitVarname(str, keyname) {
    // If the keyname is already safe for a MoonBit variable name, go ahead and use it.
    if (keyname && keyname.match(/^[a-z][a-zA-Z0-9_]+$/)) {
  ...
  function isToJsonFieldNameIdentical(safeMoonBitVarname, keyname) {
    // No translation appears to be performed by @json.ToJson.
    // if (safeMoonBitVarname !== keyname) {
    //   console.log(`isToJsonFieldNameIdentical: safeMoonBitVarname ('${safeMoonBitVarname}') != keyname ('${keyname}')`)
    // }
    return safeMoonBitVarname === keyname
  }
