$(function () {
  ...
  const emptyOutputMsg = "MoonBit will appear here"
  ...
  function doConversion() {
    ...
    let output = jsonToMoonBit(input, "", !$('#inline').is(':checked'), false, $('#omitempty').is(':checked'))
...
function constructJSONErrorHTML(rawErrorMessage, errorIndex, json) {
  ...
  var markedPart = '<span class="json-moonbit-faulty-char">' + json[errorIndex] + '</span>'
  ...
  for (var i = 0; i < jsonStringLines.length; i++) {
    ...
    if (jsonStringLines[i].indexOf('<span class="json-moonbit-faulty-char">') > -1)  // faulty line
